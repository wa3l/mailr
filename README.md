...
1. By `python mailr.py`. **Note that you need to set your API keys in `.bashrc`**. See the **Environment Variables** section for more information.
...
The subject line and body fields are required, and thus have a minimum length of 1 character. The subject line's maximum length is [78](http://www.faqs.org/rfcs/rfc2822.html). The body field has an upper limit of [25MB](http://documentation.mailgun.com/user_manual.html#sending-via-api).
...
## Demo
A demo app is live [here](http://uberemail.herokuapp.com). Please contact me for the API key for the `/emails` endpoint used for viewing saved emails.

## Notes:
1. Don't forget to set your content-type to `application/json` in the request headers. The `email` endpoint will only respond to JSON POST requests.
2. I used `urllib2` for making the POST requests originally and have working code that be seen in the commit history. However, I opted to use [Requests](http://docs.python-requests.org/en/latest/) because it simplifies the logic and makes code more readable.
3. More test coverage is coming.
